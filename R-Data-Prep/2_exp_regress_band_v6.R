...
# =================================================================
# start at above 7254
# cls.1 <- cc.cls.1[start:end.new, ]

# start.new at above 9000
...
#   }
# }
crosspt <- 67409  # between class 1 and 2.
...
band.limit3.1 <- 3*sd(cls.1$Vol.Day)
band.limit3.2 <- 3*sd(cls.2$Vol.Day)
band.limit3.3 <- 3*sd(cls.3$Vol.Day)

band.limit4.1 <- 4*sd(cls.1$Vol.Day)
band.limit4.2 <- 4*sd(cls.2$Vol.Day)
band.limit4.3 <- 4*sd(cls.3$Vol.Day)
...
# New cross point between class 2 and 3 (at red start)
range.cls.2 <- (crosspt+1):(cls.3[1, 6]-1)
range.cls.3 <- (cls.3[1, 6]):nrow(cc.data)
...
pred.band.col3 <- data.frame(c(pred.exp.1-band.limit3.1, pred.exp.2-band.limit3.2, 
                              pred.exp.3-band.limit3.3))
pred.band.col4 <- data.frame(c(pred.exp.1-band.limit4.1, pred.exp.2-band.limit4.2, 
                              pred.exp.3-band.limit4.3))

cc.data.band$pred.exp <- pred.col[, 1]
cc.data.band$pred.band <- pred.band.col[, 1]
cc.data.band$pred.band3sd <- pred.band.col3[, 1]
cc.data.band$pred.band4sd <- pred.band.col4[, 1]

#write.csv(cc.data.band, file="cc_data_band_234sd_7K_2.csv")
#write.csv(cc.data.band[, c(8, 9)], file="cc_data_band_2.csv")
# ============================================================================
cc.data.band <- read.csv("cc_data_band_234sd_9K_1.csv", row.names = 1)
...
plot.def <- ggplot(data=cc.data, aes(x=time))+
  ...
  ggtitle(sprintf("K Means Clustering for %d Classes and 2*SD Bands", num.cluster)) + labs(color = "Classes") +
...
# Plot for 2 3 4 SD bands
plot.def <- ggplot(data=cc.data, aes(x=time))+
  geom_point(aes(y=Vol.Day, color = factor(cc.data$km.cls)))+
  ggtitle(sprintf("K Means Clustering for %d Classes and 2/3/4*SD Bands (9K-1)", num.cluster)) + labs(color = "Classes") +
  theme(plot.title = element_text(lineheight=0.7, face="bold", hjust=0.5)) +
  xlab("Time") + ylab("Volume - Calendar Day Production") +
  geom_line(color='black',data = cc.data.band, aes(y=pred.exp), size=1) +
  geom_line(color='red',data = cc.data.band, aes(y=pred.band), size=1) +
